...
export function JournalPage() {
  ...
  return (
    <>
      {/* SECTION 1 — HERO (PUBLICATION STRIKE) */}
      <section aria-labelledby="journal-hero-heading" className="relative min-h-[80vh] flex flex-col justify-center items-center text-center px-6 md:px-16 pt-32 pb-24 md:pb-32 bg-neutral overflow-hidden">
        {/* IMAGE SLOT 01 — JOURNAL */}
        ...
        <motion.div variants={scrollRevealVariants} initial="hidden" animate="visible" className="relative z-10 space-y-8 max-w-3xl mx-auto">
          ...
          <h1 id="journal-hero-heading" className="font-display text-[clamp(2.5rem,6vw,5rem)] font-medium leading-[1.1] text-primary">
          ...
          <div className="space-y-4 max-w-prose mx-auto font-body text-body text-secondary">
            <p>This is not a blog. This is not a newsletter. This is not content.</p>
            <p className="text-primary font-medium">
              This is a sovereign publication — a masculine, geometric, identity-anchored briefing designed to reinforce your return.
            </p>
          </div>
          <div className="pt-8">
            <ButtonPrimary href="#subscribe">Read the Journal</ButtonPrimary>
          </div>
        </motion.div>
      </section>

      {/* SECTION 2 — WHAT THE JOURNAL DOES */}
      <section className="bg-surface py-24 md:py-36 px-6 md:px-16 border-t border-secondary/10">
        <div className="max-w-4xl mx-auto text-center">
          <motion.div variants={scrollRevealVariants} initial="hidden" whileInView={animate ?? "visible"} viewport={{ once: true }} className="mb-16 space-y-6">
            <h2 className="font-display text-4xl md:text-5xl font-medium text-tertiary">Identity Reinforcement</h2>
          ...
          <motion.div variants={staggerContainerVariants} initial="hidden" whileInView={animate ?? "visible"} viewport={{ once: true }} className="flex flex-wrap justify-center gap-x-8 gap-y-4 font-display text-xl md:text-2xl text-primary font-medium mb-16">
            {["Identity", "Purpose", "Direction", "Sovereignty", "Emotional clarity", "Decision-making"].map((item, idx) => (
          ...
          <motion.div variants={scrollRevealVariants} initial="hidden" whileInView={animate ?? "visible"} viewport={{ once: true }}>
            <p className="font-body text-body text-primary font-medium">This is identity maintenance in written form.</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div variants={scrollRevealVariants} initial="hidden" whileInView={animate ?? "visible"} viewport={{ once: true }} className="mb-16 space-y-6">
            ...
            <div className="font-body text-body text-secondary max-w-prose mx-auto space-y-4">
              ...
              <p>The Journal creates:</p>
            </div>
          </motion.div>
          <motion.div variants={staggerContainerVariants} initial="hidden" whileInView={animate ?? "visible"} viewport={{ once: true }} className="space-y-4 font-display text-2xl md:text-3xl text-primary font-medium text-center mb-16">
          ...
          <motion.div variants={scrollRevealVariants} initial="hidden" whileInView={animate ?? "visible"} viewport={{ once: true }}>
            <p className="font-body text-body text-primary font-medium uppercase tracking-[0.16em]">This is how identity becomes posture.</p>
          </motion.div>
        </div>
      </section>

      {/* SECTION 4 — CTA */}
      <section id="subscribe" className="bg-surface py-24 md:py-36 px-6 md:px-16">
